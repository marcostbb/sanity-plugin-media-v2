...
const uploadSanityAsset$ = (assetType: 'file' | 'image', file: File, hash: string) => {
  ...
  return of(null).pipe(
    ...
    mergeMap(() => {
      // @ts-ignore
        return client.observable.assets.upload(assetType, file, {extract: ['blurhash', 'exif', 'location', 'lqip', 'palette'], preserveFilename: true}).pipe(mergeMap(event => event.type === 'response' ? from(addTagToAsset(event.body.document._id, tagsRefs[market])).pipe(map(() => ({
